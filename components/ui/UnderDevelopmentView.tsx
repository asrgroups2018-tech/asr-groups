      ...
      <div className="bg-white rounded-3xl p-8 sm:p-12 border border-[#EBE7DF] shadow-[0_4px_20px_rgba(0,0,0,0.03)] text-center space-y-6 relative overflow-hidden">
        ...
        <div className="space-y-2 max-w-lg mx-auto relative z-10">
          ...
          <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
            This module is scheduled in the upcoming enterprise release. You can manage live operations in{' '}
            <strong className="text-slate-900">Dashboard, Loans, Customers, Companies, Schedule, and Administration</strong>.
          </p>
        </div>
        ...
        <div className="pt-6 border-t border-slate-100 text-xs text-slate-500 flex flex-wrap items-center justify-between gap-3 relative z-10">
          <span className="font-mono text-[11px]">
            Release Schedule: Enterprise Suite v2.4
          </span>
          <span className="text-[11px]">
            Active User: <strong className="text-slate-800">{currentActor.name}</strong> ({roles.find(r => r.id === simulatedRoleId)?.name})
          </span>
        </div>
      </div>
    </div>
